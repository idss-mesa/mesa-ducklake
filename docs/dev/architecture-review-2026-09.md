...
| M10 | high | Found by the live e2e run: after `ds_use_ticket`, **every** later operation by that identity runs under the ticket, including mesa-mcp's DuckLake Parquet push. The ticket in the run was a write ticket on the project root, and the push to `.mesa/ducklake/` failed with `no permission to update collection`. The AVU reached iCAT, but no snapshot was recorded (`ducklake_mirror_failed`). The M1 fix therefore records `via_ticket` correctly in its unit tests, but on a real Data Store a ticket-mediated change never reaches DuckLake at all. | **Open, needs a design decision.** Options: push history with a session that has no ticket bound, or with a service identity (in hosted mode the ticket holder usually has no write access to `.mesa/ducklake/`); or scope the ticket to the AVU write only. `ticket_provenance` fails until then. |
