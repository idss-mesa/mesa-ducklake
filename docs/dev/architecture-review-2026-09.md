...
| # | Sev | Finding | Outcome |
|---|---|---|---|
| D1 | high | `AvuChange` accepted `actor=""` / `source=""`, although CLAUDE.md says provenance is mandatory and must be rejected at the model layer. A blank actor would be written permanently to append-only Parquet. | **Fixed.** `field_validator` rejects empty or whitespace `actor`/`source` (`models.py`); tested in `test_models.py`. |
